...
class Magnet(object):
    ...
    def bctrl(self, val):
        ...
        if not isinstance(val, float) or isinstance(val, int):
            self._logger.info('you need to provide an in or float')
            return
    ...
    def length(self, length):
        ...
        if not isinstance(length, float):
            self._logger.info('You must provide a float for magnet length')
            return
    ...
    def tol(self, tol):
        ...
        if not isinstance(tol, float):
            self._logger.info('You must provide a float for magnet tol')
            return False
    ...
    def add_clbk(self, fn, attr='_bact'):
        ...
        if attr not in self._pv_attrs:
            self._logger.info('this attribute is not a pv object, ignored')
            return

        fns = [val[0] for val in getattr(self, attr).callbacks.values()]

        if fn in fns:
            self._logger.info('this is a duplicate callback assignment, ignored')
            return

        self._logger.info('adding callback {0}'.format(fn))
    ...
    def remove_clbk(self, fn, attr='_bact'):
        ...
        if not index:
            self._logger.info('function not found in callbacks, ignored')
            return
